...
from data_collect.binary import fbin_read, fvecs_save, fvecs_read
...
# base_data = fbin_read(f'../data/text2image1B/base.1B.fbin.crop_nb_10000000')
# query_data = fbin_read(f"../data/text2image1B/query.public.100K.fbin")
...
#     fvecs_save(f'../data/{dataset}.{i}.{distribution1}.train.itv', create_interval(fvecs_read(f'../data/{dataset}.{i}.train.fvecs').shape[0], "uniform", left=0, right=1000))
#     fvecs_save(f'../data/{dataset}.{i}.{distribution2}.test.itv', create_interval(fvecs_read(f'../data/{dataset}.{i}.test.fvecs').shape[0], "uniform", left=0, right=100))
...
search_params_1, search_params_2 = [[ef, iter] for ef in range(10, 200, 20) for iter in range(1, 10, 2)], \
    [[ef, iter, sim/100] for ef in range(10, 30, 4) for iter in range(1, 10, 2) for sim in range(0, 101, 20)]
